            ...
            >
              {filteredGuides.length > 0 ? (
                filteredGuides.map((guide, index) => (
                  ...
                  >
                    <Link href={guide.link} className="block overflow-hidden h-48">
                      <Image 
                        className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" 
                        alt={guide.title} 
                        src="https://images.unsplash.com/photo-1561784541-9a06ac2eb2ea"
                        width={400}
                        height={200}
                      />
                    </Link>
                    <div className="p-6 flex flex-col flex-grow">
                      ...
                      <h3 className="font-heading text-lg font-semibold mb-3 text-foreground leading-tight flex-grow">
                        <Link href={guide.link} className="hover:text-primary transition-colors">{guide.title}</Link>
                      </h3>
                      <p className="text-foreground/80 mb-4 leading-relaxed text-sm">
                        {guide.excerpt}
                      </p>
                      <Link href={guide.link} className="text-primary hover:text-primary-hover font-semibold group-hover:text-primary-hover flex items-center transition-colors mt-auto">
                        Read More
...
// Metadata for client components should be in layout or separate metadata file
